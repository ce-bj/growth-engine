...
| 字体 | 现代无衬线、经典衬线、技术等宽 |
| 可选输入 | 主动作调整、导入意图、知识目录、产品、文档、图片、表单及业务字段 |
...
“使用模板”与“自定义组件”互斥显示。模板选中后显示名称和状态；生成按钮直接创建首版。高级设置放在任务输入附近，不作为全流程常驻的顶部入口。创建页及编辑页的对话框下方提供“导入意图”；高级设置不再提供“同步访客意图”，避免重复入口。
...
导入文件先校验，再显示来源、意图、模板、执行方式与推广摘要。点击“导入并生成”创建任务，不覆盖当前草稿；生成后直接进入编辑与预览，显示已关联意图。原型限制 JSON 最大 1 MB。
...
原型记录原始请求、意图标识与指纹，使用固定报告展示检测结果。手动上传文件后停留在编辑与预览，由用户继续配置发布；文件中的自动发布策略保留供正式 Agent 接口使用，不由手动导入自动执行。没有真实鉴权、资料解析、外部生成和发布服务。内置素材与授权示例值只用于原型。
